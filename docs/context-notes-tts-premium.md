...
## 배경
TikDo.exe가 Naver CLOVA + Typecast를 무료 플랜으로 제공하는 패턴을 tikke에 이식. 출처 분석 — [[reference_tikdo_tts]].

## 아키텍처 결정
- **worker 프록시 방식** (TikDo식 "키를 클라에 내림" 금지). 키는 worker secret에만. desktop은 worker 엔드포인트 호출.
