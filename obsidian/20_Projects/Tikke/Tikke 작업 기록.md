...
- 결정: `Tikke_comp` 백업은 소스코드·비밀정보·앱 설정 제외, 문서와 Obsidian Markdown 선별 동기화 정책을 계속 유지한다.
- 작업: `tikke` 문서/설계 파일 56개를 `Tikke_comp/docs/`에 재동기화하고 변경 없음으로 커밋을 생략했다.
- 작업: Obsidian 볼트 Markdown 84개를 `Tikke_comp/obsidian/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드·비밀정보·제외 디렉토리 위반 0건, 타겟 파일 56개로 확인했다.
- 확인: Obsidian 동기화는 민감정보 제외 0건, 기타 제외 0건, 커밋 `3c20e02`와 clean 상태로 확인했다.
- 다음: Windows 경로 처리 실패 후 재시도된 사례가 있어 `/c/...`와 `C:/...` 경로 사용 차이를 계속 주기 점검한다.

## 2026-07-20

- 결정: `Tikke_comp` 백업은 소스코드·비밀정보·앱 설정 제외, 문서와 Obsidian Markdown 선별 동기화 정책을 계속 유지한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 GitHub 원격에 푸시했다.
- 작업: Obsidian 볼트 Markdown 84개를 `Tikke_comp/obsidian/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자 및 비밀키·토큰 패턴 위반 0건, 커밋 `0da15cf`로 확인했다.
- 확인: Obsidian 동기화는 민감정보 패턴 제외 0건, `.obsidian`·`.trash` 미복사, Markdown 외 파일 0개, 커밋 `4c72a83`와 clean 상태로 확인했다.
- 다음: 자동 동기화 제외 규칙과 private repo 전제를 계속 주기 점검한다.

## 2026-07-21

- 결정: `Tikke_comp/docs` 동기화에서 `.venv`, `venv`, `env`, `__pycache__` 같은 생성물·캐시 디렉토리를 제외 규칙에 포함한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 임시 동기화 스크립트는 제거했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀키·토큰 패턴 및 제외 디렉토리 위반 0건, 타겟 파일 58개, Git 상태 clean으로 확인했다.
- 다음: 문서 동기화가 필요한 문서를 유지하면서 Python 가상환경 산출물을 계속 제외하는지 주기 점검한다.

## 2026-07-22

- 결정: `Tikke_comp` 백업은 문서/Obsidian Markdown만 선별하고 소스코드·비밀정보·앱 설정 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 docs 변경 없음으로 커밋을 생략했다.
- 작업: Obsidian 볼트 Markdown 84개를 `Tikke_comp/obsidian/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀정보 패턴·제외 디렉토리 위반 0건, 타겟 파일 58개로 확인했다.
- 확인: Obsidian 동기화는 Markdown 외 파일 0개, `.obsidian`·`.trash` 미복사, 민감정보 패턴 제외 1건, 커밋 `206be06`와 clean 상태로 확인했다.
- 다음: `Tikke_comp` 자동 백업에서 필요한 Markdown 누락과 민감정보 과포함 여부를 계속 주기 점검한다.

## 2026-07-23

- 결정: `Tikke_comp/docs` 동기화는 문서/설계 파일만 복사하고 소스코드·비밀정보·생성물 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀정보 패턴 복사 0건, 제외/프루닝 디렉토리 19건, 커밋 `62c8b3d`와 Git 상태 clean으로 확인했다.
- 다음: `Tikke_comp/docs` 자동 동기화에서 필요한 문서 누락과 비밀정보 과포함 여부를 계속 주기 점검한다.

## 2026-07-24

- 결정: `Tikke_comp/docs` 동기화는 문서/설계 파일만 복사하고 소스코드·비밀정보·생성물 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀키/토큰/패스워드/credential 패턴 파일 0건, 커밋 `2e2ec0b`와 Git 상태 clean으로 확인했다.
- 다음: `Tikke_comp/docs` 자동 동기화에서 필요한 문서 누락과 비밀정보 과포함 여부를 계속 주기 점검한다.

## 2026-07-25

- 결정: `Tikke_comp` 백업은 문서/Obsidian Markdown만 선별하고 소스코드·비밀정보·앱 설정 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 58개를 `Tikke_comp/docs/`에 재동기화하고 변경 없음으로 커밋을 생략했다.
- 작업: Obsidian 볼트 Markdown 84개를 `Tikke_comp/obsidian/`에 재동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀정보 패턴 파일 0건, 타겟 파일 58개로 확인했다.
- 확인: Obsidian 동기화는 `.obsidian`·`.trash` 미복사, 민감정보 패턴 제외 1건, 커밋 `5a86d50`과 clean 상태로 확인했다.
- 다음: `Tikke_comp` 자동 백업에서 필요한 Markdown 누락과 민감정보 과포함 여부를 계속 주기 점검한다.

## 2026-07-26

- 결정: `Tikke_comp/docs` 동기화는 문서/설계 파일만 선별하고 소스코드·비밀정보 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 60개를 `Tikke_comp/docs/`에 재동기화하고 신규 문서 2개를 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀키/토큰/패스워드/credential 패턴 파일 0건, 커밋 `ac2bde3`와 Git 상태 clean으로 확인했다.
- 다음: `Tikke_comp/docs` 자동 동기화에서 새로 추가되는 작업 문서 누락과 민감정보 과포함 여부를 계속 주기 점검한다.

## 2026-07-27

- 결정: `Tikke_comp/docs` 동기화는 `.venv`, `venv`, `env`, `__pycache__` 같은 생성물·캐시 디렉토리를 제외하고 문서/설계 파일만 유지한다.
- 작업: `tikke` 문서/설계 파일 60개를 `Tikke_comp/docs/`에 재동기화했으며 변경 없음으로 커밋과 푸시는 생략했다.
- 확인: 1차 동기화에서 `.venv` 유입 가능성을 발견해 제외 규칙을 보강했고, 최종 검증은 타겟 파일 60개, 소스코드 확장자·비밀정보 패턴·제외 디렉토리 위반 0건, Git 상태 clean으로 확인했다.
- 다음: 생성물 디렉토리 제외 규칙이 유지되는지 계속 주기 점검한다.

## 2026-08-23

- 결정: `Tikke_comp` 백업은 문서/Obsidian Markdown만 선별하고 소스코드·비밀정보·앱 설정 제외 정책을 유지한다.
- 작업: `tikke` 문서/설계 파일 127개를 `Tikke_comp/docs/`에 동기화하고 GitHub 원격에 푸시했다.
- 작업: Obsidian 볼트 Markdown 85개를 `Tikke_comp/obsidian/`에 동기화하고 GitHub 원격에 푸시했다.
- 확인: 문서 동기화는 소스코드 확장자·비밀키/토큰/credential 패턴·제외 디렉토리 위반 0건, 커밋 `711ca70`로 확인했다.
- 확인: Obsidian 동기화는 민감정보 패턴 제외 0건, `.obsidian`·`.trash`·백업 파일 미복사, 커밋 `e9979b8`과 Git 상태 clean으로 확인했다.
- 다음: `Tikke_comp` 자동 백업에서 필요한 문서 누락과 민감정보 과포함 여부를 계속 주기 점검한다.

## 2026-08-24

- 결정: `Tikke_comp/docs` 동기화는 `.venv`, `venv`, `env`, `__pycache__`, `.pytest_cache` 같은 생성물·캐시 디렉토리를 제외하고 문서/설계 파일만 유지한다.
- 작업: `tikke` 문서/설계 파일 127개를 `Tikke_comp/docs/`에 재동기화했으며 변경 없음으로 커밋과 푸시는 생략했다.
- 확인: 동기화 검증은 타겟 파일 127개, 소스코드 확장자 위반 0건, 비밀정보 패턴 위반 0건, `NO_DOCS_CHANGES`로 확인했다.
- 다음: 생성물 디렉토리 제외 규칙과 민감정보 과포함 여부를 계속 주기 점검한다.
